            ...
            <table id="table_id" class="table table-bordered">
              ...
              <tbody>
                ...
                <tr>
					...
					<?php 
					 if($currdate <= $mydate && $value->status  <= 8)
					 {?>
                    ...
                    <?php if($value->type == '9') 
						  ...
						  {
                    ?>
                    <a class="btn btn-rounded btn-warning">Waiting For Client Approval</a>
                    <?php }else{?>
                          <a href="<?= base_url('Request/request_detail?value=') . $value->visit_id ?>" class="btn btn-rounded btn-info">View</a>
                          <?php if ($value->status == '1' or $value->status == '3') { ?>
                          <a href="<?= base_url('Sister_Request/AssignedSister?id=') . $value->visit_id ?>" class="btn btn-rounded btn-info">Assign SisterLab</a>
                          <?php } else { ?>
                          <?php } ?>
                          <?php if($value->status == '10' || $value->status == '9' || $value->status == '12') {
                          } else { ?>
                          <a title="Cancel Request" href="<?= base_url('Sister_Request/cancelled?id=') . $value->visit_id ?>" class="btn btn-rounded btn-danger">
