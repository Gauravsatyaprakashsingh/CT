              ...
              <tbody>
                <?php $counter = 1;  foreach ($tableData as $key => $value):  ?>
                  <tr>
                    ...
                    <td><?=$value->pincode?></td>
                    <td>
                      <?php if( $value->isPhelboAssigned == '0' && $value->status == '2' ): ?>
                        <a href="<?=base_url('Task/accept_request?requestId=').$value->visit_id.'&status=1' ?>"  title="Accept Request" class="btn btn-icons btn-rounded btn-success"> <i class="fa fa-check"></i></a>
                        <a href="<?=base_url('Task/accept_request?requestId=').$value->visit_id.'&status=2' ?>"  title="Reject Request" class="btn btn-icons btn-rounded btn-danger"> <i class="fa fa-times"></i></a>
                      <?php elseif( $value->isPhelboAssigned == '1' && $value->status == '3'  ): ?>
