...
class Samples extends My_Controller{
	...
	public function add_patient()
	{
		...
		$sql_query = "SELECT * FROM `visit_schedule` where visit_unique_id='{$ID}'";
		$details_visit = $this->Master_model->rawQuery($sql_query);
		$array_chk = json_decode(json_encode($details_visit[0]), true);
		$data['project_id'] = $array_chk['project_id'];
		$data['visit_id'] = $array_chk['visit_id'];
		$data['visit_unique_id'] = $array_chk['visit_unique_id'];
	...
	public function Remainder_email()
	{
	   ...
	   if(!empty($visitid))
	   {
		   ...
		   $requestor_id = $this->session->userdata['log_user']['user_id'];
		   $email[] = '';
		   $data_chk = $this->Casual_Model->get_details($visitid);
		   $data_sample = $this->Casual_Model->get_sample_collected($visitid);
		   if(!empty($data_chk)){
			    $email[] = $data_chk['Representative_email'];
		   }
		   if(!empty($data_sample)){
			    $isSampleRequested = $data_sample['sample_id'];
		   }
		   $this->sendRequestmail($requestor_id,$userType,$isSampleRequested,$email,$havingAttachment,$attachment,$visitid);
	   }
	}
    ...
    public function savecamp(){
		...
		$visit_unique_id = $post['visit_unique_id'];
		$requestor_id = $this->session->userdata['log_user']['user_id'];
		$userType  = $this->session->userdata['log_user']['type'];
		$this->load->model('Casual_Model');
		...
		{
			    $totalPatient = count($post['patientUniqueId']);
				if(!empty($totalPatient)){
				
					for($i=0; $i<$totalPatient ; $i++){
						 $uniqueId = $post['patientUniqueId'][$i];
						 $pateintData =['patient_name' => $post['patient_name'][$i],
						         'patient_contact' => $post['contact'][$i],
								 'patient_email' => $post['email'][$i],
								 'visit_id' => $visitID,
								 'patient_age' => $post['ages'][$i],
		...
		$sql = "SELECT *,c.type_name FROM sample_collection sc INNER JOIN patient p ON p.patient_id = sc.patient_id
				INNER JOIN test_master t ON t.test_id = sc.test INNER JOIN type_of_payment c on sc.payment_status = c.type_id
			 	WHERE sc.visit_id = '{$visitID}'";
		$testList = $this->Master_model->rawQuery( $sql );
		$viewData = [];
		foreach ($testList as $key => $value) {
					...
					$this->session->set_flashdata('success',"Requested");
					//$this->loadView('Samples/add_patient?id='.$visit_unique_id,$data);
					redirect('Samples/add_patient?id='.$visit_unique_id);
	...
	public function sendRequestmail($userId,$userType,$visitID,$email,$havingAttachment=false,$attachment =[],$visit_id){
